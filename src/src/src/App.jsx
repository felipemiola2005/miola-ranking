  ...
  return (
    <div style={{ background: "#0d0202", minHeight: "100vh", fontFamily: "Lato,sans-serif", color: "#fff", maxWidth: 480, margin: "0 auto" }}>
      ...
      <div style={{ padding: 16, paddingBottom: 48 }}>
        {activeTab==="geral" && (
          <div>
            ...
            <div style={S.secTitle}>Ranking Geral — {filterMode==="month"?getMonthLabel(filterMonth):"Periodo"}</div>
            {globalRanking.map((entry,i) => <RankingRow key={entry.barber.id} entry={entry} pos={i+1} isAdmin={isAdmin} onDelete={requestDelete} currentSales={filteredSales} />)}
          </div>
        )}
        {activeTab==="unit" && (
          <div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 16 }}>
              {UNITS.map(u => <button key={u} style={{ background: selectedUnit===u?"#8B0000":"rgba(255,255,255,0.04)", color: selectedUnit===u?"#FFD700":"#888", border: `1px solid ${selectedUnit===u?"#FFD700":"#2a0a0a"}`, borderRadius: 20, padding: "6px 12px", fontSize: 12, cursor: "pointer", fontFamily: "Oswald,sans-serif" }} onClick={() => setSelectedUnit(u)}>{u}</button>)}
            </div>
            <div style={S.secTitle}>Ranking — {selectedUnit}</div>
            {unitRanking.map((entry,i) => <RankingRow key={entry.barber.id} entry={entry} pos={i+1} isAdmin={isAdmin} onDelete={requestDelete} currentSales={filteredSales} />)}
          </div>
        )}
        {activeTab==="history" && (
          <div>
            <div style={S.secTitle}>Historico de Meses</div>
            {historyMonths.length===0 && <div style={{ color: "#555", textAlign: "center", padding: 32 }}>Nenhum historico disponivel ainda.</div>}
            {historyMonths.map(m => {
              const ms=getMonthSales(sales,m);
              const top=computeRanking(ms,barbers).filter(e => e.count>0).slice(0,3);
              return (
                <div key={m} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: 16, marginBottom: 12 }}>
                  <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 17, color: "#FFD700", fontWeight: 700, marginBottom: 4 }}>{getMonthLabel(m)}</div>
                  <div style={{ color: "#aaa", fontSize: 13, marginBottom: 8 }}>{ms.length} vendas · {fmtCurrency(ms.reduce((a,s) => a+s.planValue,0))}</div>
                  {top.map((entry,i) => <div key={entry.barber.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}><Medal pos={i+1} /><span style={{ color: "#eee", fontSize: 14 }}>{entry.barber.name}</span><span style={{ color: "#FFD700", fontSize: 13, marginLeft: "auto" }}>{entry.count} assin. · {fmtCurrency(entry.revenue)}</span></div>)}
                </div>
              );
            })}
          </div>
        ...
        {activeTab==="mysales" && currentBarber && (() => {
          ...
          return (
            <div>
              <div style={S.secTitle}>Minhas Vendas</div>
              <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
                {[["Assinaturas",mySales.length],["Faturado",fmtCurrency(total)]].map(([l,v]) => <div key={l} style={{ flex: 1, background: "rgba(255,215,0,0.07)", border: "1px solid rgba(255,215,0,0.2)", borderRadius: 12, padding: "14px 12px", textAlign: "center" }}><div style={{ fontFamily: "Oswald,sans-serif", fontSize: 22, fontWeight: 700, color: "#FFD700" }}>{v}</div><div style={{ fontSize: 11, color: "#888", marginTop: 2 }}>{l}</div></div>)}
              </div>
              {mySales.length===0 ? <div style={{ color: "#555", textAlign: "center", padding: 32 }}>Nenhuma venda no periodo.</div> :
                mySales.sort((a,b) => new Date(b.date)-new Date(a.date)).map(sale => (
                  <div key={sale.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                    <div><div style={{ fontWeight: 600, color: "#eee", fontSize: 14 }}>{sale.clientName}</div><div style={{ color: "#aaa", fontSize: 12 }}>{sale.plan}</div><div style={{ color: "#666", fontSize: 11 }}>{sale.origin} · {new Date(sale.date).toLocaleDateString("pt-BR")}</div></div>
        ...
        {activeTab==="admin" && isAdmin && (
          <div>
            <div style={{ ...S.adminBox, marginBottom: 20 }}>
              <div style={S.secTitle}>📊 Origem das Vendas</div>
              <DonutChart data={originStats} />
            </div>
            <div style={{ ...S.adminBox, marginBottom: 20 }}>
              <div style={S.secTitle}>💰 Resumo por Unidade</div>
              {unitSummary.map(u => <div key={u.unit} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}><div><div style={{ color: "#eee", fontWeight: 600, fontSize: 14 }}>{u.unit}</div><div style={{ color: "#888", fontSize: 12 }}>{u.count} assinaturas</div></div><div style={{ color: "#FFD700", fontWeight: 700, fontSize: 15 }}>{fmtCurrency(u.revenue)}</div></div>)}
            </div>
            <div style={S.adminBox}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <div style={S.secTitle}>👤 Barbeiros</div>
                <button style={{ background: "#FFD700", color: "#0d0202", border: "none", borderRadius: 8, padding: "7px 14px", fontFamily: "Oswald,sans-serif", fontWeight: 700, fontSize: 13, cursor: "pointer" }} onClick={() => setShowAddBarber(true)}>+ Novo</button>
              </div>
      ...
      {showAdminDelete && (
        <div style={S.overlay}><div style={S.modal}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <div style={{ fontFamily: "Oswald,sans-serif", fontSize: 20, color: "#FFD700", fontWeight: 700 }}>Confirmar Exclusao</div>
            <button style={{ background: "rgba(255,255,255,0.1)", color: "#fff", border: "none", borderRadius: "50%", width: 30, height: 30, fontSize: 14, cursor: "pointer" }} onClick={() => setShowAdminDelete(false)}>✕</button>
          </div>
          <div style={{ color: "#aaa", fontSize: 14, marginBottom: 16 }}>Digite a senha de exclusao para confirmar:</div>
          <input style={S.input} type="password" value={adminDeletePin} onChange={e => setAdminDeletePin(e.target.value)} placeholder="Senha" />
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            <button style={{ flex: 1, background: "rgba(255,255,255,0.07)", color: "#aaa", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: 14, fontSize: 15, fontFamily: "Oswald,sans-serif", cursor: "pointer" }} onClick={() => setShowAdminDelete(false)}>Cancelar</button>
            <button style={{ ...S.btn, background: "#8B0000" }} onClick={confirmDelete}>Excluir</button>
          </div>
        </div></div>
      )}
